...
def test_read_and_summarize_with_url_routes_browser():
    """INTEGRAÇÃO — inverte a expectativa original da track BH.

    O teste original exigia NORMAL para "Leia e resuma esse artigo: <url>",
    partindo de que fetch resolveria fora do specialist. Mas NORMAL é slim e
    tem 0 tools: ninguém buscaria a URL, e o modelo responderia um resumo
    inventado de uma página que nunca leu. Isso é pior que rotear.

    A primeira use case do doc 00 §4.1 é exatamente "Leia e resuma estes
    materiais" + links. A escolha entre fetch e navegador é o decision gate
    do §4.2, aplicado DENTRO de browser.read, onde existe a evidência da
    página — não no roteador por keyword. Roteador decide QUEM atende, a
    capability decide COMO busca.
    """
    assert resolve_specialist_route("Leia e resuma esse artigo: https://exemplo.com/post") == "BROWSER"


def test_read_and_summarize_without_url_stays_normal():
    """Sem URL explícita não há o que buscar — segue conservador em NORMAL."""
    assert resolve_specialist_route("Leia e resuma esses links pra mim") == "NORMAL"
